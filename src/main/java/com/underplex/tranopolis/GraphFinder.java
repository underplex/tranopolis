...
public class GraphFinder {
	 ...
	 * @return
	 */
	public static Map<Xing, Set<Lot>> findPavedNeighbors(City city) {
		...
		Map<Xing, Set<Lot>> rXings = new HashMap<Xing, Set<Lot>>();
		for (Lot lot : xings.keySet()){
			rXings.put(new Xing(lot), xings.get(lot));
		}
		return rXings;
	}
	...
	public static RoadGraph findRoadGraph(City city) {
		...
		for (Road r : findRoadsWithXings(city, rXings)) {
			graph.addRoad(r);
		}
		return graph;
	}
